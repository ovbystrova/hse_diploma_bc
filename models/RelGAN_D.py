...
class RelGAN_D(nn.Module):
    def __init__(self, embed_dim, max_seq_len, num_rep, vocab_size,
                 dis_num_filters, dis_filter_sizes, weights, dropout=0.25):
    ...
    def forward(self, inp):
        ...
        cons = [F.relu(conv(emb), inplace=False) for conv in
                self.convs]  # [batch_size * num_filter * (seq_len-k_h+1) * num_rep]
